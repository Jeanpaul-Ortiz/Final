...
bool Card::sameRankAs(const Card &c2) const {
    ...
    if(this->getRank() == c2.getRank()) return true;
...
bool Card::sameRank(const vector<Card> &deck) const {
    ...
    for(int i = 0; i < deck.size(); i++){
        if(deck[i].getRank() != this->getRank()) return false;
    }
    return true;
}
...
Card Card::maxRankCard(const vector<Card> &deck) {
    if(deck.empty()) return Card(JOKER, SPADES);
    Card max = deck[0];
 ...
 * Suit's is as follows:  SPADES < HEARTS < DIAMONDS < CUBS
 * 
...
bool Card::isFourOfAKind(const vector<Card> &hand) {
    ...
    // map<Rank, int> mapHand;
    // sort(myHand.begin(), myHand.end(), TWO);
    // int counter = 1;
    // for(int i = 0; i < myHand.size(); i++){
    //     if(myHand[i].getRank() == myHand[i+1].getRank()){
    //         counter++;
    //     }
    //     else{mapHand.insert(pair(myHand[i].getRank(), counter));
    //         counter = 1;
    //     }
    // }

    // if(mapHand.size() > 2) return false;

    // return true;
    if(hand.size() < 5) return false;
    map<Rank, int> mapHand;
    for(int i = 0; i < hand.size(); i++){
        mapHand.insert(pair<Rank, int>(hand[i].getRank(), 1));
    }
...
set<Card> getIntersection(const set<Card> &deck1, const set<Card> &deck2) {
    ...
    return {Card(Rank(0), Suit(0))}; // Dummy return
}
...
vector<Card> Card::allOfSuit(const vector<Card> &deck, Suit s) {
    ...
    return {Card(Rank(1), Suit(1))}; // Dummy return
}
